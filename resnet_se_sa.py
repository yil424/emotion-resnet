...
class ResNet18SE_SA(nn.Module):
    def __init__(self, num_classes: int = 5, pretrained: bool = True):
        ...
        # ResNet18 four stages
        self.layer1 = backbone.layer1
        self.layer2 = backbone.layer2
        self.layer3 = backbone.layer3
        self.layer4 = backbone.layer4

        # Channel attention (SE) on top of layer4 output
...
def main():
    ...
    model = ResNet18SE_SA(num_classes=len(class_names), pretrained=True).to(device)
    criterion = nn.CrossEntropyLoss(label_smoothing=0.1)
    optimizer = optim.Adam(model.parameters(), lr=learning_rate)

    best_val_acc = 0.0
    best_model_path = os.path.join(project_dir, "resnet18_se_sa_best.pth")
    ...
    best_model = ResNet18SE_SA(num_classes=len(class_names), pretrained=False).to(device)
