...
def load_model(checkpoint_path: str):
    ...
    model = ResNet18SE(num_classes=len(EMOTION_LABELS), pretrained=False).to(device)
...
def generate_emotion_response_llm(emotion: str) -> str:
    prompt = (
        f"You are a friendly assistant that reacts to a user's facial emotion.\n"
        f"The detected emotion is: {emotion}.\n\n"
        f"Write a short message directly to the user. "
        f"If they seem sad or worried, be warm and encouraging (tell them a tiny joke). "
        f"If they seem happy or surprised, be playful and positive. "
        f"Do NOT mention that you were given an emotion label.\n"
    )
    ...
    try:
        ...
        print(f"[WARN] LLM call failed, fallback to template. Error: {e}")
...
ckpt_path = os.path.join(os.path.dirname(__file__), "resnet18_se_best.pth")
...
if st.session_state.show_result and st.session_state.result is not None:
    ...
    st.markdown(
        '<div class="footer-text">Model: ResNet18-SE • Local LLM: qwen3:8b via Ollama</div>',
        unsafe_allow_html=True,
    )
